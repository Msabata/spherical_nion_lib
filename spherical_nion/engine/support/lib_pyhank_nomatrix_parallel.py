# This library is a compy of pyhank library, while it replaces creation of transformation matrix to save memory.
...
class HankelTransform:
    ...
    def __init__(self, order: int, max_radius: float = None, n_points: int = None,
        ...
        # Calculate some vectors for hankel transform
        jp1 = np.abs(scipy_bessel.jv(order + 1, self.alpha))
        self.JR = jp1 / self.max_radius
        self.JV = jp1 / self.v_max
        # no_matrix self variables:
        self.jp1 = jp1

    def calculate_fv(self, c_fv, alpha_S, _fr):
        T_line = scipy_bessel.jv(self._order, (alpha_S * alpha_S[c_fv]))
        return np.dot(T_line, _fr)    
        

    def qdht(self, fr: np.ndarray) -> np.ndarray: #T changed qdht_nomatrix to qdht
        """QDHT: Quasi Discrete Hankel Transform
        
        Performs the Hankel transform of a function of radius, returning
        a function of wavenumber (k_z). Avoids creating transformation matrix to save computer memory.

        Input
        -----
        fr : 1D array
            Radial function (sampled at self.r)

        Returns
        -------
        fv : 1D array
            Function in reciprocal space (sampled at self.kr)
        """
        print(">>> qdht_nomatrix")
        _fr = np.array(fr) # make copy to not revrite originall array
        jr, jv = self._get_scaling_factors(_fr)
        fv = np.zeros(self._n_points, dtype=complex)
        alpha_S = self.alpha / self.S**0.5
        #######################################
        # acceleration by enlighting the matrix
        #######################################
        _fr /= (jr*self.jp1) # rescaling

        with Pool() as pool:
            fv = pool.starmap(self.calculate_fv, [(c_fv, alpha_S, _fr) for c_fv in range(self._n_points)])

        fv *= 2 * jv / (self.S*self.jp1)
        return fv

    #method used by Pool() in iqdht_nomatrix to parallelize computations on CPU
    def calculate_fr(self, c_fr, alpha_S, _fv):
        T_line = scipy_bessel.jv(self._order, (alpha_S * alpha_S[c_fr]))
        return np.dot(T_line, _fv)  

    def iqdht(self, fv: np.ndarray) -> np.ndarray:
        """IQDHT: Inverse Quasi Discrete Hankel Transform
        
        Performs the inverse Hankel transform of a function of wavenumber (k_z), returning
        a function of radius. Avoids creating transformation matrix to save computer memory.
        ...
        """
        print(">>> iqdht_nomatrix")
        _fv = np.array(fv)
        jr, jv = self._get_scaling_factors(_fv)
        fr = np.zeros(self._n_points, dtype=complex)
        alpha_S = self.alpha / self.S**0.5
        _fv /= (jv*self.jp1) # rescaling

        with Pool() as pool:
            fr = pool.starmap(self.calculate_fr, [(c_fr, alpha_S, _fv) for c_fr in range(self._n_points)])

        fr *= 2 * jr / (self.S*self.jp1)
        return fr
