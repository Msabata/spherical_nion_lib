...
class HankelTransform:
    ...
    def qdht(self, fr: np.ndarray) -> np.ndarray:
        ...
        for c_fv in range(self._n_points):
            T_line = scipy_bessel.jv(self._order, (alpha_S * alpha_S[c_fv]))
            fv[c_fv] = np.dot(T_line,_fr)
    ...
    def iqdht(self, fv: np.ndarray) -> np.ndarray:
        ...
        for c_fr in range(self._n_points):
            T_line = scipy_bessel.jv(self._order, (alpha_S * alpha_S[c_fr]))  # only one row of the transforamtion matrix
            fr[c_fr] = np.dot(T_line,_fv)
